      ...
      {sizes.map((s) => {
        ...
        return (
          <div key={s} className={styles.sizeSegment}>
            <div
              className={styles.sizeSegBar}
              style={{
                background: past ? info.color : 'var(--border)',
                opacity: active ? 1 : past ? 0.5 : 0.3
              }}
            />
            <span
              className={styles.sizeSegLabel}
              style={{
...
function NeedsChart({ formData }) {
  const bars = needsKeys
    .map(({ key, label }) => {
      const idx = lvls.indexOf(formData[key] || '')
      return {
        label,
        pct: idx <= 0 ? 0 : (idx / (lvls.length - 1)) * 100,
        color: bClrs[Math.max(idx, 0)],
        val: formData[key] || ''
      }
    })
    .filter((b) => b.pct > 0)
    ...
    <div className={styles.needsChart}>
      {bars.map((b) => (
        <div key={b.label} className={styles.needsRow}>
          ...
          <div className={styles.needsTrack}>
            <div
              className={styles.needsFill}
...
export default function FinalPage({ state, vertical, onBack }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      console.log('[Wiseness Intake] Payload:', payload)
    }
    setSending(false)
    setSent(true)
  }
      ...
      <div className={styles.successPage}>
        <div className={styles.successCard}>
          <div className={styles.successIconWrap}>
            <Icon name="check" size={28} color="var(--status-green)" />
          </div>
          <h1 className={styles.successTitle}>Talep gönderildi</h1>
          <p className={styles.successText}>
            Sistem yapılandırmanız başarıyla gönderildi. Wiseness ekibi kısa süre içinde sizinle iletişime geçecektir.
          </p>
          {formData.firmenname && (
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.left}>
            ...
            <div className={styles.analysisRow}>
              ...
              <div className={styles.analysisMid}>
                ...
                <div className={styles.sizeReadout}>
                  <span className={styles.sizeCode} style={{ color: size.color }}>
                    {systemSize}
                  </span>
          ...
          <div className={styles.card}>
            <div className={styles.cardHead}>
              ...
              <span>Önerilen modüller</span>
              <span className={styles.countTag}>{modules.length}</span>
            </div>
          ...
          <div className={styles.card}>
            <div className={styles.cardHead}>
              ...
              <span>Önerilen sensörler</span>
              <span className={styles.countTag}>{sensors.length}</span>
            </div>
            <div className={styles.sensorTable}>
              <div className={styles.sensorTableHead}>
                <span>Tür</span>
                <span>Miktar</span>
              </div>
              {sensors.map((s) => (
          ...
          <div className={styles.card}>
            <div className={styles.cardHead}>
              <Icon name="target" size={14} color="var(--text-muted)" />
              <span>İhtiyaç analizi</span>
            </div>
            ...
            <ul className={styles.hintList}>
              {technicalHints.map((h, i) => (
                <li key={i} className={styles.hintItem}>
                  <div className={styles.hintNum}>{i + 1}</div>
                  {h}
                </li>
              ))}
            </ul>
        ...
        <div className={styles.right}>
            ...
            <table className={styles.sumTable}>
              <tbody>
                {[
                  ...
                  ['Çalışan sayısı', formData.anzahlMitarbeiter],
                  ['Kullanıcı sayısı', formData.anzahlBewohner],
                  ['Bütçe', formData.budgetrahmen],
                  ['Öncelik', formData.prioritaet],
                  ['Zaman planı', formData.umsetzungszeitraum],
                  ['Sektör tipi', vertical?.title],
                  ['Sistem boyutu', `${systemSize} – ${size.label}`, true],
                  ['Önerilen kamera sayısı', `${cameraCount} adet`, true],
          ...
          {formData.freitextWuensche && (
            <div className={styles.card}>
              <div className={styles.cardHead}>
                <Icon name="messageSquare" size={14} color="var(--text-muted)" />
                <span>Özel istekler</span>
              </div>
          ...
          <div className={styles.card}>
            ...
            <p className={styles.sendText}>
              Tüm yapılandırma, düzenli bir veri kaydı olarak Wiseness ekibine iletilir.
            </p>
